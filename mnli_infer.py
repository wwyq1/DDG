...
dataset = load_dataset("/root/ww-xd/LongICLBench/mnli/")
train_data = dataset['train']
test_data = dataset['test']
demo_data = select_data(given_dataset=train_data, number_of_turns=args.round)
eva_data = select_data(given_dataset=test_data, number_of_turns=500)
total = 0
correct = 0
...
for example in eva_data[:args.test_number]:
    #cur_prompt = demo_prompt + "service query: "+ '\n' + example['premise'] + '\n' + example['hypothesis'] + "\nintent category: " 
    cur_prompt = demo_prompt + "service query: "+ '\n' + example['premise'] + '\n' + example['hypothesis']
    ...
    if example['hypothesis'] in response:
        response = list(response.split(example['hypothesis']))[-1].strip().split(temp_prompt)
        if len(response) > 1:
            response = response[1]
        else:
            response = response[0]
    ...
    response = re.sub(r'[“”,."]', r'', response.strip())
    response = response.lower().strip()
    print("pred: ", response)
    label = all_labels [example['labels']]
    label = re.sub(r'[“”,."]', r'', label.strip())
    label = label.lower()
    print("labels: ", label)
    if response == label:
        correct += 1
    ...
    print("all: ", total)
    output_dict = {}
    output_dict['premise'] = example['premise']
    output_dict['hypothesis'] = example['hypothesis']

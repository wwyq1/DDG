...
class DataModule:
    ...
    def get_dataset(self):
        ...
        if os.path.exists(self.config.datasets_path):
            ...
            datasets = load_dataset("SetFit/sst5")#*self.dataset_attr["load_args"])
    ...
    def get_learner_train_loader(
        ...
        drop_last: bool = True,
        label: int | None = None,
        ddp: bool = False,  # 添加一个参数用于判断是否启用分布式训练
        **kwargs,
    ):
        if dataset is None:
        ...
        if ddp:
            # 在分布式环境下使用 DistributedSampler
            sampler = DistributedSampler(dataset)
            shuffle = False  # 分布式采样器会自动进行 shuffle
        else:
            sampler = None
    ...
    def valid_loader(self, ddp: bool = False) -> DataLoader:
        assert "validation" in self.preprocessed_datasets
        assert self.data_collator is not None

        if ddp:
            sampler = DistributedSampler(self.preprocessed_datasets["validation"], shuffle=False)
        else:
            sampler = None

...
def mlflow_start_run_with_hydra(func):
    ...
    def wrapper(config: Config, *args, **kwargs):
        ...
        with mlflow.start_run(run_name=config.base.run_name):
            ...
            import platform
            mlflow.log_params(
                {"hostname": platform.uname()[1], "device": torch.cuda.get_device_name()}
            )
...
def main(config: Config):
    ...
    learner_1 = LearnerModel(config.learner, task_name=config.data.task_name)

...
class TrainerDC(TrainerBase):
    def fit(
       ...
       # assert outer_loop % (self.config.log_interval // self.config.inner_loop) == 0
        ...
        #     batch_sentences = []
        #     batch_labels = []
        #     for idx in range(len(batch_learner['text'])):
        #         sentence = batch_learner['text'][idx]
        #         label = batch_learner['labels'][idx]
        #         aug_sentences = eda(sentence, alpha_sr=0.1, alpha_ri=0.1, alpha_rs=0.1, p_rd=0.1, num_aug=4)
        #         for aug_sentence in aug_sentences:
        #             batch_sentences.append(aug_sentence)
        #             batch_labels.append(label)
        #     batch_sentences = learner.tokenizer(batch_sentences, padding=True, max_length=learner.tokenizer.model_max_length, truncation=True, return_tensors='pt')
        #     batch_sentences['labels'] = torch.tensor(batch_labels, dtype=torch.int64)
        #     outputs = learner(**batch_to_cuda(batch_sentences))
        ...
        for ol in trange(
            ...
            if (
                ...
                gm_syn_loaders = self.get_gm_syn_loaders(
                    generator, learner_1, data_module
                )
            ...
            for outer_step in trange(
                ...
                if (outer_step + 1) < self.config.inner_loop:
                    for _ in range(self.config.model_step_per_inner_step):
                        ...
                        #with torch.no_grad():  # 不计算梯度，直接更新参数
                        for param_1, param in zip(learner_1.parameters(), learner.parameters()):
                            param_1.data = 0.99 * param_1.data + 0.01 * param.data
                            
            gradients = learner.get_last_layer_gradients()
            for param_name, grad in gradients.items():
                logger.info(f"Last layer {param_name} gradient: mean={grad.mean():.6f}, std={grad.std():.6f}")

...
class GeneratorModel(nn.Module):
    ...
    def save_model(self, save_path):
        ...
        if not ddp or dist.get_rank() == 0:
            ...
            return self.model.save_pretrained(save_path) if ddp else self.model.save_pretrained(save_path)

    def load_model(self, save_path):
        logger.info(f"Load generator model from `{save_path}`.")
        self.model = self.model.from_pretrained(save_path) if ddp else self.model.from_pretrained(save_path)

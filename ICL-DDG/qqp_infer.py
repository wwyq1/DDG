...
dataset = load_dataset("/root/ww-xd/LongICLBench/qqp/")
...
for example in eva_data[:args.test_number]:
    ...
    if example['question2'] in response:
        response = list(response.split(example['question2']))[-1].strip().split(temp_prompt)
        if len(response) > 1:
            response = response[1]
        else:
            response = response[0]
    ...
    print("all: ", total)
    output_dict = {}
    output_dict['question1'] = example['question1']
    output_dict['question2'] = example['question2']

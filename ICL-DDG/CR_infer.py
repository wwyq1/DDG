...
def select_data(given_dataset, number_of_turns):
    ...
    for data in given_dataset:
        if data['labels'] in label_to_data_dict:
            label_to_data_dict[data['labels']].append(data)
        else:
            label_to_data_dict[data['labels']] = [data]
...
def format_discovery_prompt(data_dict_list, round=0, with_instruction=False, context_token_number="2k"):
    ...
    for data in data_dict_list[:index]:
        prompt = prompt + "service query: "+ data['text'] + "\nintent category: " + all_labels[data['labels']] + '\n'
    return prompt
...
dataset = load_dataset("/root/ww-xd/LongICLBench/CR/")
train_data = dataset['train']
test_data = dataset['test']
demo_data = select_data(given_dataset=train_data, number_of_turns=args.round)
eva_data = select_data(given_dataset=test_data, number_of_turns=50)
total = 0
correct = 0
...
if args.round != 0:
    if args.instruct:
        output_file = f'sst2_round_instruct_result/{args.model}_{args.round}.json'
    else:
        output_file = f'sst2_round_result/{args.model}_{args.round}.json'
else:
    if args.instruct:
        output_file = f'sst2_instruct_result/{args.model}_{args.context_length}.json'
    else:
        output_file = f'sst2_result/{args.model}_{args.context_length}.json'
...
for example in eva_data[:args.test_number]:
    #cur_prompt = demo_prompt + "service query: " + example['sentence'] + "\nintent category: "
    ...
    print("pred: ", response)
    label = all_labels[example['labels']]
    label = label.lower()
    print("labels: ", label)
    if response == label:
        correct += 1
